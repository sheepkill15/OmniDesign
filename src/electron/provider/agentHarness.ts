...
export function createDesignAgentInstructions(workspacePath: string, attachments: readonly Attachment[] = [], sourceProjectPath: string | null = null, conversationRecap = ''): string {
  ...
  return [
    ...
    '- An Alpine <template> directive such as x-if or x-for requires exactly one root element inside the <template>. Multiple sibling roots do not work; wrap them in one element.',
