...
describe('preview shim injection', () => {
  ...
  it('supports focused selection through opaque source keys and suppresses authored clicks', () => {
    ...
    expect(result).toContain("querySelectorAll('[data-od-source-key]')")
    expect(result).toContain("type: 'focused-anchors'")
    expect(result).toContain('rect.top >= viewportHeight')
