...
- macOS packaged-runtime repairs on 2026-08-04 keep authenticated Claude usable when its CLI help formatting changes by parsing unquoted aliases and falling back to the officially documented `sonnet` and `opus` aliases; provider readiness now consistently requires a usable model. Tailwind compilation resolves from the packaged compiler module instead of Finder's `/` working directory, and the trusted production renderer permits required runtime style attributes through `style-src-attr` while retaining `script-src 'self'` and blocking inline `<style>` blocks. Windows verification: `pnpm typecheck`, 304 unit/component tests across 32 files, production build, all 10 Playwright Electron journeys, Windows packaging, and a Tailwind compile loaded directly from the packaged `app.asar` while launched from `C:\`. Native packaged macOS verification remains required.
- Workspace isolation now remounts renderer state when the active design changes, preventing a previous design's popped-out mode from briefly opening the next design before its own docked layout loads. The generated-design agent contract also records Alpine's single-root requirement for `x-if`/`x-for` templates. Verification on 2026-08-04: `pnpm typecheck`, 305 unit/component tests across 32 files, production build, and the built Electron pop-out/dock journey pass.
