...
- Generation now enters a persisted job queue. The initial coordinator runs up to two designs concurrently while maintaining a sequential queue per design, retains job state in SQLite, and marks queued or running work interrupted on application shutdown or restart. Stop, Continue, Retry, automatic retries, self-repair, and user-facing queue management remain the next additions to this subsystem.
