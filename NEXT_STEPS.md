...
- Dropdown controls now share one primitive: `src/renderer/components/MenuButton.tsx` (a React Aria trigger button with an attached popover), reused by the generation-settings, project-selector, and layout-mode controls. It is the first extracted shared UI component and a step toward the planned `packages/ui`; new button-with-dropdown controls should use it rather than hand-rolling `MenuTrigger`/`Popover`. The revision-history panel remains a bespoke popover for now (its custom outside-click/Escape dismissal predates this primitive); folding it onto a shared popover primitive is a reasonable later cleanup.
