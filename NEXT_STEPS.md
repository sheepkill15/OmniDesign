...
- Focused selection preserves the isolated preview boundary: generated code receives only opaque source keys and cannot claim paths, lines, revisions, or excerpts. Selecting an element opens a trusted feedback popup beside it. Each matched element retains one subtle anchored thread marker combining pending and submitted focused comments in chronological order; hover or keyboard focus reveals the thread, and only pending entries can be removed. Historical targets follow later revisions only through a unique same-page stable identity or unchanged exact source, never fuzzy matching. **Fix all** submits every ordered comment and exact target as one provider turn and one generation attempt. Resolved batch metadata persists in message and attempt history while live selection remains ephemeral.
