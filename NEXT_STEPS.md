...
- Dropdown controls are shared components in `src/renderer/components/`. There are two, chosen by context:
  - `DropdownButton.tsx` — a plain-DOM popover with a rotating caret, CSS `:hover` rows, and capture-phase outside/Escape dismissal. Non-modal by nature, so it behaves correctly **above the isolated preview's native layer**, where React Aria's modal overlays contend for focus (breaking focus-driven hover) and inert the background. Used by the header **revision-history** and **layout-mode** controls; its `onOpenChange` drives the preview freeze. Any new dropdown that can sit over the preview must use this.
  - `MenuButton.tsx` — a React Aria `MenuTrigger`/`Popover`. Used only by the composer's **generation-settings** and **project-selector** controls, which never overlap the preview, so React Aria's keyboard/menu semantics are a net win there.
  If the two ever need to unify, migrate the composer controls onto `DropdownButton` (the generation-settings panel would keep its effort slider and swap its provider/model React Aria menus for plain rows).
