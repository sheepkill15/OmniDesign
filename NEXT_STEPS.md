...
- Projects are now first-class in the trusted UI. The sidebar and home "Continue designing" list persisted projects (ordered by recent activity) rather than individual designs; opening a project applies the specified direct-open-versus-grid behavior (empty or multi-design projects open a project page with a preselected new-design composer and a design grid, while a single-design project opens straight into its workspace). Linking a local folder deduplicates onto the existing project instead of registering a duplicate, and a project may hold multiple designs. Project/design trash and removal, linked-folder availability recovery, and Git clone remain to be built on this model.
