...
- Focused selection preserves the isolated preview boundary: generated code receives only opaque source keys and cannot claim paths, lines, revisions, or excerpts. A selected element can be submitted immediately or its comment can join a durable conversation-side queue; **Fix all** submits every ordered comment and exact target as one provider turn and one generation attempt. Resolved batch metadata persists in message and attempt history while live selection remains ephemeral.
