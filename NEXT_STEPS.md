...
- The isolated preview is a native layer composited above the DOM, so trusted-UI overlays that sit over a docked preview (the header layout menu and revision-history dropdown) would otherwise be hidden behind it. While such an overlay is open the renderer asks the main process to `setVisible(false)` on the preview view (a ref-counted suspend that only affects a docked preview, not a detached or popped-out one), and restores it on close. This avoids the previous approach of hosting every dropdown in its own window. If future overlays are positioned over the preview, wire them into the same `useSuspendPreviewWhileOpen` signal.
