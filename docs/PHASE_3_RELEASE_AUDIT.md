...
| Requirement | Implementation and verification |
| --- | --- |
| Single target in Focused mode | The preview toolbar exposes a pressed-state selection toggle only for the head revision. Once enabled, repeated elements remain selectable until the user explicitly disables it or leaves the design; Escape clears the current pick and Canvas only suspends picking. Pointer selection, Enter/Space selection, authored-action suppression, hover/focus outline, and explicit clearing are implemented. Selection focuses a collision-fitted trusted feedback popup immediately beside the element instead of moving the user to the conversation composer. |
