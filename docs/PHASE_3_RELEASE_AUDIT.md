...
| Requirement | Implementation and verification |
| --- | --- |
| Single target in Focused mode | The preview toolbar starts selection only for the head revision. Canvas activation switches to Focused first. Pointer selection, Enter/Space selection, Escape cancellation, authored-action suppression, hover/focus outline, target chip, and explicit clearing are implemented. |
