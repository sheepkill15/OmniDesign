...
| Requirement | Implementation and verification |
| --- | --- |
| Single target in Focused mode | The preview toolbar exposes a pressed-state selection toggle only for the head revision. Once enabled, repeated elements remain selectable until the user explicitly disables it, presses Escape, switches to Canvas, or leaves the design. Pointer selection, Enter/Space selection, authored-action suppression, hover/focus outline, and explicit clearing are implemented. Selection opens a collision-fitted trusted feedback popup immediately beside the element instead of moving the user to the conversation composer. |
